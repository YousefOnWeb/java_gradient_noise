...
{
	vec4 tex_val_hi = texture3D(shader_noise_tex, position.xyz);
    // hi.r is noise1, hi.g is noise2 (formerly hi.a)
	vec2 hi = 2.0 * tex_val_hi.rg - 1.0; 
	
    vec4 tex_val_lo = texture3D(shader_noise_tex, position.xyz / 9.0);
...
fbmnoise(vec3 position, int octaves)
{
	...
	for (int x = 0; x < octaves; x++) {
        vec4 tex_val = texture3D(shader_noise_tex, p.xyz);
